...
class AsyncRolloutRequest(BaseModel):
    ...
    # ========================================
    # === Abnormal-trajectory flag fields (for PPO training stability) ===
    # ========================================
    #
    # These fields flag abnormal conditions encountered during rollout, for
    # the downstream PPO Trainer / Reward Manager to handle specially.
    #
    # Classification of abnormalities:
    # 1. Discarded (excluded from training):
    #    - tool_parse_error: JSON parsing failed / unknown tool (incomplete trajectory)
    #    - search_error: API retries exhausted (environment failure)
    #    - exceed_max_turns: exceeded the max turn limit (incomplete)
    #    - exceed_max_tokens: exceeded the token limit (truncated)
    #
    # 2. Reward zeroed for the offending round only (trajectory kept):
    #    - excessive_tool_calls_per_turn: too many queries in a single turn; does not break, continues execution
    #
    # 3. Handled normally (does not affect reward):
    #    - repeated_query: duplicate search
    
    # Discard-level flags
    tool_parse_error: bool = False               # JSON parse failure / malformed tool args -> discard
    search_error: bool = False                   # flagged after all MAX_RETRIES API retries fail -> discard
    exceed_max_turns: bool = False               # reached max_assistant_turns -> discard
    exceed_max_tokens: bool = False              # truncated after exceeding max_model_len -> discard
    
    # Round-level zeroing flag
    excessive_tool_calls_per_turn: bool = False  # tool calls in a single turn > max_tool_calls_per_turn (only that round's reward is zeroed)
    
    # Normal-handling flag
    repeated_query: bool = False                 # current query exactly duplicates a prior query (reward computed normally)
    
    # Monitoring statistics
    total_assistant_turns: int = 0               # total number of assistant messages (search steps)
    total_tool_calls: int = 0                    # total number of tool calls (sum of all query_list lengths)
    total_tool_call_attempts: int = 0            # total number of <tool_call> tags seen (including failed parses)
    max_tool_calls_in_single_turn: int = 0       # max number of queries issued in a single turn
    tool_calls_per_turn: list[int] = []          # per-turn query count list (query_list length per turn)
    query_history: list[str] = []                # history of all queries issued (for duplicate detection)
    unknown_tool_names: list[str] = []           # list of unknown tool names (for monitoring undefined tool calls by the model)
    
    # finish_reason_type (for downstream analysis)
    finish_reason_type: Optional[FinishReasonTypeEnum] = None

    def check_repeated_query(self, new_query: str) -> bool:
        """Check whether the current query duplicates a prior query.
        ...
        """
        # Normalize the query: strip whitespace and lowercase
        normalized_query = new_query.strip().lower()

        # Check for an exact duplicate against the query history
        for history_query in self.query_history:
            if normalized_query == history_query.strip().lower():
                return True
        return False

    def add_queries_to_history(self, queries: list[str]) -> None:
        """Add a list of queries to the history (batch append).
    ...
    def initialize_request(cls, values):
        ...
        if (
            ...
            if values["input_ids"].shape[-1] > max_prompt_len:
                ...
                print(
                    f"[DEBUG] Prompt {values['batch_data_id']} has length {values['input_ids'].shape[-1]} "
                    f"which is greater than max_prompt_len {max_prompt_len} after applied chat template with tools."
                )
    ...
    def add_tool_response_messages(
        ...
        for content in contents:
            ...
            if content.text:
                ...
                try:
                    resp = str(json.loads(content.text).get("result", ""))
                except Exception as e:
                    logger.error(f"[schemas.add_tool_response_messages] Request {self.request_id}: JSON parse error - {e}")
                    traceback.print_exc()
                    resp = ""

            self.messages.append(Message(role="tool", content=resp))
    ...
    def finalize(
        ...
        if finish_reason_type == FinishReasonTypeEnum.STOP:
            ...
            pass
        elif finish_reason_type == FinishReasonTypeEnum.TOOL_CALL:
            # ========================================
            # === Edge case: finalize while in TOOL_CALL state ===
            # ========================================
            # Trigger condition:
            # - rollout was in the TOOL_CALLING state, but some abnormal
            #   condition (unknown tool, parse error, etc.) caused a break
            #   out of the loop without finish_reason_type being correctly
            #   reset to STOP.
            #
            # Handling:
            # - Log a warning (marked as abnormal)
            # - Treat this as an abnormal trajectory, but do not raise
            # - Continue with the finalize flow
            logger.warning(
                f"[Abnormal] Request {self.request_id} finalize with TOOL_CALL finish_reason. "
                f"This indicates an abnormal rollout (e.g., unknown tool, parse error). "
                f"tool_parse_error={self.tool_parse_error}, search_error={self.search_error}"
            )
        else:
            raise ValueError(
